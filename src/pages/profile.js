...
function Profile() {
  ...
  const { data, isPending, error } = useFetch2(
    `https://pharma-mobile-backend.herokuapp.com/profile/${pk}`
  );
  ...
  const NameChange = async (e) => {
    ...
    const response = await fetch(
      `https://pharma-mobile-backend.herokuapp.com/name_change/`,
      {
        method: "POST",
        headers: {
              ...
              {orders.map((order) => {
                return (
                  <div className="single-person most-recent-nav" key={order.id}>
                    ...
                    <h6>{order.price} $</h6>
                    <button
                      id={`pay-cash-${order.id}`}
                      name={`pay-cash-${order.id}`}
                      className="search-button"
                    >
                      pCash
                    </button>
                    <button
                      id={`pay-card-${order.id}`}
                      name={`pay-card-${order.id}`}
                      className="search-button"
                    >
                      pCard
                    </button>
            ...
            <div className="people-list">
              ...
              {children
                ? children.map((child) => {
                    return (
                      <div
                        className="single-person all-dependents-nav"
                        key={child.id}
                      >
                        <h6>
                          <span>{child.id}</span>
          ...
          <div className="people-list">
            ...
            {orders ? (
              orders.map((order) => {
                return (
                  <div
                    className="single-person transactions-history-nav"
                    key={order.id}
                  >
                    <h6>
                      <span>{order.customer.id}</span>
